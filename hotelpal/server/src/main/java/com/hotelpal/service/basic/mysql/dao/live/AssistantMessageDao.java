...
@Component
public class AssistantMessageDao extends ExtendedMysqlBaseDao<AssistantMessageSO, AssistantMessagePO> {
	private static final String TABLE_NAME = TableNames.TABLE_ASSISTANT_MESSAGE;
	private static final List<String> TABLE_COLUMNS_LIST = new ArrayList<>(Arrays.asList(("id,createTime,updateTime," +
			"deleted,liveCourseId,msg").split(",")));//Total 6.
	private static final Map<String, Integer> TABLE_COLUMN_MAP = tableListToMap(TABLE_COLUMNS_LIST);
	@Override
	protected Class<AssistantMessagePO> getPOClass() {
		return AssistantMessagePO.class;
	}
	@Override
	...
	@Override
	protected void searchNonColumnField(StringBuilder buff, List<Object> params, AssistantMessageSO so, String alias) {
	}
	
	public List<AssistantMessagePO> getMessageList(Integer courseId) {
		AssistantMessageSO so = new AssistantMessageSO();
		so.setPageSize(null);
		so.setLiveCourseId(courseId);
		return this.getList(so);
	}
}
