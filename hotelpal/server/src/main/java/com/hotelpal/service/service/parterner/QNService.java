...
public class QNService {
	...
	private static final String PERSISTENT_NOTIFY_URL = PropertyHolder.getProperty("QN_PERSIST_NOTIFY_URL");
		...
		try {
			...
			if(doCompress) {
				...
				upToken = auth.uploadToken(bucketName, key, 10 * 60, new StringMap().put("persistentOps", "avthumb/mp3/aq/7|saveas/" + urlbase64).
						putNotEmpty("persistentPipeline", AUDIO_PIPELINE).put("persistentNotifyUrl", PERSISTENT_NOTIFY_URL).put("force", 1));
			} else {
